...
private:
    ...
    {
        if (fSubTrees[aSubtreeIndex]->empty()) throw std::domain_error("Subtree is empty");
        if (aSubtreeIndex >= 3) throw std::out_of_range("Illegal subtree index");
    ...
    {
        if (empty()) throw std::domain_error("Empty tree");
        if (aSubtreeIndex >= 3) throw std::out_of_range("Illegal index");
        if (!fSubTrees[aSubtreeIndex]->empty()) throw std::domain_error("Non-empty subtree present");
...
public:
	...
	{
        if (empty()) throw std::domain_error("Tree is empty");
        return fKey;
	}
...
// Problem 2: TernaryTree Copy Semantics
	...
	TTree& operator=( const TTree& aOtherTTree )
	{
        if (this != &aOtherTTree)
            ...
            else throw std::domain_error("Cannot copy NIL");
        return *this;
	}
...
// Problem 3: TernaryTree Move Semantics
	...
	TTree& operator=( TTree&& aOtherTTree )
	{
        if (this != &aOtherTTree)
            ...
            else throw std::domain_error("Cannot move NIL");
	}
